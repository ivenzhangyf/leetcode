class Solution {
    public List<String> findWords(char[][] board, String[] words) {
        ...
        for(String s:words){
            if (isword(board,s)&&!res.contains(s))
                res.add(s);
        }
        return res;
    }
    ...
    public boolean dfs(char[][] board,char[] words,int[][] visited,int x,int y,int index){
        ...
        visited[x][y]=1;
        if(dfs(board,words,visited,x+1,y,index+1)||dfs(board,words,visited,x,y+1,index+1)||dfs(board,words,visited,x-1,y,index+1)||dfs(board,words,visited,x,y-1,index+1))
            return true;
        visited[x][y]=0;
        return false;
    }
